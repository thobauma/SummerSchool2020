...
class Field {
    ...
    void update_host() {
        ...
        auto status = cudaMemcpy(&device_ptr_, &host_ptr_, xdim_*ydim_*sizeof(double), cudaMemcpyDeviceToHost);
        cuda_check_status(status);
    }

    void update_device() {
        // TODO: copy contents of host_ptr_ to device_ptr_ 
        auto status = cudaMemcpy(&host_ptr_, &device_ptr_, xdim_*ydim_*sizeof(double), cudaMemcpyHostToDevice);
        cuda_check_status(status);
    }
